"""Pilot deep training — accepted-run clean segments -> small ResGCN -> recording-level evaluation.

Design: docs/superpowers/specs/2026-08-09-pilot-deep-training-design.md

Core principles
  - Training windows are not stored on disk; each epoch takes a random crop
    within a clean segment (= phase augmentation). Evaluation uses many
    evenly-spaced windows + **recording-level majority vote**.
  - Splits are **recording-level grouped K-fold** — window/run-level splits leak.
  - The joint set (body/hand/both) and channel branches are varied via
    arguments to run contrastive experiments.
...
def load_runs(csv_path: Path, root: Path, min_seg_frames: int,
    ...
    If `only` is given, use only those conductors (to fix the class composition).
    seg_blacklist is the (conductor, run, seg_idx) set from segver.load_blacklist() —
    segments flagged as identity-contaminated are excluded from window extraction
    (indexed against qc.json).
    **Caution**: the review UI keeps updating the CSV, so comparison experiments
    across conditions MUST pin a snapshot CSV (--csv) — otherwise the data differs
    between conditions. Likewise, seg_blacklist should use a copy taken at
    snapshot time.
    """
    out = []
    n_seg_dropped = 0
    ...
    if seg_blacklist:
        print(f"segver: excluded {n_seg_dropped} contaminated segment(s)")
    return out
...
def _video_quality(runs: list[dict],
    ...
    Used as a proxy for video quality since resolution metadata isn't available
    (the original videos aren't kept) — same definition as rec_conf() in
...
def _fore_ratio(run_dir: Path, native_fps: float, tgt_len: int,
    ...
    Taylor (2000, CVIU 80): under scaled orthographic projection, the on-screen
    length is ell = s * L * cos(theta). Dividing by the per-frame shoulder width
    removes the scale factor (s); dividing by the run median removes body build
    (L), leaving only the relative value of r ~ cos(theta) — i.e. how much the
    bone is tilted out of the screen plane (toward/away from the camera). This
    single channel reinjects the depth-direction motion that normalization
    erased. The sign (toward/away) is inherently ambiguous from a single
    viewpoint (per the same paper), so it is not provided. Invalid frames (low
    confidence / shoulders not detected) get the neutral value 1. Clipped to
    [0.2, 2.0].
    """
    ...
    for pj, cj in BONES:
        ...
        # anchor: median = only the relative change from the person's baseline /
        # p95 = the 'most extended-looking moment' ~ true length (Taylor: at the
        # instant it's parallel to the screen, ell -> s*L) -> r approaches the
        # absolute cos(theta), preserving even the habitual tilt (static style).
        # Trade-off: camera angle gets mixed in.
...
class RunCache:
    """Loads normalized skeletons into memory (tens of minutes total).

    skeleton_norm.npy is **already saved at 25fps** by the normalize stage — if
    we resample it again here, the time axis gets double-compressed and no
    longer lines up with segment coordinates (a real bug that happened before:
    resampling again based on the qc fps (native) -> the last 17% of windows for
    a 29.97fps run turned into mirror-reflection garbage, and all windows ran
    20% too fast).

    If depth=True, the foreshortening ratio is computed from the raw coordinates
    and appended as a 4th column. lowpass applies an extra low-pass filter (for
    ...
    def __init__(self, runs: list[dict], lowpass: float = 0.0,
        ...
        self.seq = {}
        for r in runs:
            a = np.load(r["npy"]).astype(np.float32)   # already at 25fps
            if lowpass:
...
def _channels_padded(w: np.ndarray, joints: str, use: tuple, drop_conf: bool,
    ...
    **Pad after computing channels**: velocity/acceleration derivatives are
    finished using only the real frames, and only afterward is the shortfall at
    the end filled with zeros. Reversing the order (zero-pad first, then
    differentiate) bakes a spurious 'fake sudden acceleration' — several times
    larger than real motion — into the channel at the real/padding boundary.
    The mask is 1=observed / 0=padded, used to correct the pooling denominator.
    """
    if joints == "pair":
    ...
    def __getitem__(self, i):
        ...
        if self.frame_drop and len(w) > 1 and self.rng.random() < 0.5:
            ...
            for _ in range(k):   # frame drop = replace with the previous frame
                ...
                w[j] = w[j - 1]
        x, m = _channels_padded(w, self.joints, self.use, self.drop_conf, self.T)
        return torch.from_numpy(x), self.labels[r["conductor"]], torch.from_numpy(m)


class EvalWindows(Dataset):
    """Evenly-spaced windows — also returns run/recording identifiers, used for majority-vote aggregation."""
    ...
    def __init__(self, runs, cache, labels, joints, use, T, n_eval,
                 ...
                 eval_margin=0, pad="none"):
        # Cap on windows per recording — keeps long recordings from dominating window-level metrics.
        by_rec: dict = {}
        for r in runs:
    ...
    def __getitem__(self, i):
        ...
        if self.local_hands:
            w = CH.localize_hands(w)
        if self.shuffle_frames:      # S1 control: if it still gets it right after destroying order, it's reading a static cue
            w = w[self.rng.permutation(len(w))]
        x, m = _channels_padded(w, self.joints, self.use, self.drop_conf, self.T)
        # Unseen classes (a third conductor used for the bridge test) have no label -> -1
...
class STGCNPPScratch(torch.nn.Module):
    """For architecture comparison: trains the ST-GCN++ backbone from scratch
    (no pretraining) on **our input** (normalized 51 joints, 13 channels). The
    graph uses the bone connections of our joint set (graph.edges); input
    (B,C,T,V) -> (N,M=1,T,V,C). Same (logit, emb) interface as ResGCN."""
    ...
    def __init__(self, in_channels: int, num_classes: int, joints: str, coord: bool):
        ...
        # edges is (min,max) undirected — build inward as (child, parent) based on parents
        par = CH.parents(joints)
        inward = [(c, int(p)) for c, p in enumerate(par) if c != p]
        for a, b in G.edges(joints, coord):           # reinforce non-tree edges (bilateral-correction / coordination links)
...
def supcon_loss(emb, y, tau=0.1):
    """Contrastive loss that pulls same-class samples together and pushes different-class samples apart within a batch."""
...
def run_fold(tr_runs, te_runs, cache, labels, args, device):
    ...
    # The mask is passed to the model only when pad=zeromask — "zero" is the
    # condition where the model has to figure it out on its own (the advisor's
    # original suggestion), so the zeros are fed in as-is with no denominator
    # correction.
    ...
    win_by_rec = defaultdict(list)               # per-recording window correctness (for macro average)
    win_ok = win_n = 0
    win_cm = np.zeros((len(labels), len(labels)), dtype=int)  # window-level confusion matrix
    per_run = defaultdict(lambda: [np.zeros(len(labels)), -1])  # run -> [probability sum, ground truth]
    # Track padded vs. non-padded windows separately — to see whether "the
    # data we rescued is actually being classified correctly" without it
    # blending into overall accuracy (a pre-registered reading rule).
    ...
    with torch.no_grad():
        for x, y, vid, cond, runid, m, is_pad in dl_e:
            ...
            for yt, yp in zip(y, p.argmax(1)):
                if yt >= 0:                      # exclude unseen classes (-1)
                    win_cm[yt, yp] += 1
            for k in range(len(y)):
                ...
                cur[1] = y[k]
                pr = per_run[runid[k]]           # shot (run) level — sum of probabilities
                pr[0] = pr[0] + p[k]
                pr[1] = y[k]
...
def main():
    ...
    ap.add_argument("--eval-margin", type=int, default=0,
                    help="Pull evaluation windows back by this many frames (at 25fps) from "
                         "both segment edges — boundary-artifact sensitivity experiment")
    ...
    ap.add_argument("--local-hands", action="store_true",
                    help="Subtract the wrist from hand coordinates to remove arm trajectory — finger shape only")
    ap.add_argument("--fps-only", type=float, default=0.0,
                    help="Use only runs whose native fps equals this value — blocks resampling artifacts")
    ap.add_argument("--depth-anchor", default="median", choices=["median", "p95"])
    ap.add_argument("--no-depth", action="store_true",
                    help="Turn off the foreshortening (depth) channel — ablation for depth's contribution")
    ap.add_argument("--lowpass", type=float, default=0.0,
                    help="Extra low-pass filter (Hz) — control removing fine temporal texture")
    ...
    ap.add_argument("--arch", default="resgcn", choices=["resgcn", "stgcnpp"],
                    help="Classifier architecture — stgcnpp trains from scratch on our input, no pretraining")
    ap.add_argument("--pad", default="none", choices=["none", "zero", "zeromask"],
                    help="Rescue segments shorter than the window: none=discard (original) - "
                         "zero=zero-pad the tail - zeromask=zero-pad + exclude from the pooling denominator")
    ap.add_argument("--min-real-frac", type=float, default=0.5,
                    help="Minimum segment length when using --pad (fraction of real frames "
                         "relative to the window). 0.5 = at least half the window must be observed frames")
    ...
    ap.add_argument("--seg-verdicts", default="",
                    help="Segment identity-verdict CSV — excludes contaminated segments "
                         "(a snapshot copy is recommended, e.g. data/experiments/segver_YYYYMMDD.csv)")
    ap.add_argument("--ood-exclude", default="",
                    help="Video CSV to exclude from training (conductor,video_id) — a second "
    ...
    ap.add_argument("--session-merges", default="",
                    help="Manual session-merge CSV confirmed via audio comparison "
                         "(e.g. data/experiments/session_merges_YYYYMMDD.csv)")
    ap.add_argument("--dup-drops", default="",
                    help="CSV of audio cross-correlation verdict=same pairs — drops true "
                         "duplicates (re-uploads / excerpts), keeping only the best-quality "
                         "one per cluster (e.g. data/experiments/audio_sweep_20260814.csv)")
    args = ap.parse_args()

    device = "cuda" if torch.cuda.is_available() else "cpu"
    if args.pad == "zeromask" and args.arch != "resgcn":
        ap.error("--pad zeromask is only implemented for resgcn")
    seg_bl = load_blacklist(args.seg_verdicts) if args.seg_verdicts else frozenset()
    # Using padding lowers the segment-length floor from T to
    # T*min_real_frac, bringing previously-discarded short segments into the
    # window candidate pool.
    ...
    # OOD (rehearsal etc.) videos are held out — blocked here even if they remain in the snapshot.
    ...
    if ood_excl:
        ...
        print(f"OOD exclusion ({args.ood_exclude}): {len(ood_excl)} video(s) listed -> "
              f"run {before}->{len(runs)}")
    if args.fps_only:
    ...
    if args.dup_drops:
        ...
        # Representative selection: prioritize total clean footage, break
        # ties by video quality (conf) — for excerpt vs. full-piece
        # duplicates, going by conf alone would let the short excerpt win
        # and lose unique footage.
        ...
        if drop:
            ...
            print(f"True-duplicate drop ({args.dup_drops}): excluded {len(drop)} video(s) "
                  f"(run {before}->{len(runs)})")

    labels = {c: i for i, c in enumerate(sorted({r["conductor"] for r in runs}))}
    recs = sorted({(r["conductor"], r["video_id"]) for r in runs})
    print(f"[{args.tag}] run {len(runs)} - recordings {len(recs)} - classes {labels} - {device}")

    cache = RunCache(runs, lowpass=args.lowpass, depth=not args.no_depth,
                     depth_anchor=args.depth_anchor)
    # Folds are split at the **session level** — if per-movement uploads of
    # the same performance get split across train/test, the stage, costume,
    # and camera leak the answer (a hole in video_id-level splitting).
    titles = {}
    ...
    # Keep the split at the group (session) level, but balance conductor
    # ratios across folds — with pure random assignment, classes with few
    # groups (e.g. Bernstein at 16 groups / 5 folds) were empirically
    # observed to end up with zero groups in a particular fold
    # (2026-08-25, without stratification).
    ...
    win_accs, win_macros = [], []
    win_tot = 0
    run_c = run_n = 0                            # shot (run)-level aggregate
    win_cm_tot = 0                               # window-level confusion matrix aggregate
    pad_c = pad_t = unpad_c = unpad_t = 0        # separate padded-window / non-padded-window tallies
    ...
    # chance = majority-class ratio at the recording level
    cnt = defaultdict(int)
    for c, _ in recs:
        cnt[c] += 1
    ...
    out = {"tag": args.tag, "joints": args.joints, "use": args.use,
           ...
           # window-level chance = recomputed from the actual window count since window composition varies by condition
    ...
    if args.pad != "none":
        print(f"  [{args.pad} T={args.T}] padded-window {pad_c}/{pad_t} = "
              f"{out['pad_acc']:.3f} - non-padded-window {unpad_c}/{unpad_t} = "
              f"{out['unpad_acc']:.3f} - window chance {out['win_chance']:.3f}")
        for k in sorted(pad_cls_t):
            print(f"    padded-window {inv_lab[k]:<10s} {pad_cls_c[k]}/{pad_cls_t[k]} = "
